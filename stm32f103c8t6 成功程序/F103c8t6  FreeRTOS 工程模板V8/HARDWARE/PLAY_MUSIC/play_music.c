...
printf("俯仰角Pitch : %f \t 横滚角Roll : %f \t 航向角Yaw %f \t\r\n",Att_Angle.pit*10,Att_Angle.rol*10,Att_Angle.yaw*10);
}
#endif
...
void yuying_Run(void)//语音对话
{
    ...
    if((USART1_RX_BUF[1]+USART1_RX_BUF[2])==USART1_RX_BUF[3])
    {
        ...
        {
			case 23: //ds18b20
					vTaskDelay(2500); //当前温度为
				    select_num(_value[2]);
					printf("play,021,$"); //度
//					ds18b20();	
			break;
			case 24: //dht11
					vTaskDelay(3000);//当前湿度为百分之
				    select_num(_value[0]);
			break;
			case 25: //MQ135
					vTaskDelay(3000);//当前空气质量为
					MQ135();
            break;
			case 31: //你好，很高兴认识你
					runActionGroup(10,1); //运行10号动作组1次
            ...
            break;
			case 33: 
					vTaskDelay(2500);//请欣赏音乐		
					printf("play,050,$"); //音乐《成都》
            break;
			case 34: 
					vTaskDelay(2500); //请欣赏舞蹈
					printf("play,049,$"); //音乐《小苹果》
					runActionGroup(14,1); 
            break;
//			case 49:
