...
u8   TX_ADDRESS[TX_ADR_WIDTH]= {0x34,0x43,0x10,0x10,0x02};	//发送给别人时  对方的地址（发送帧头地址）
u8   RX_ADDRESS[RX_ADR_WIDTH]= {0x34,0x43,0x10,0x10,0x01};	//接收别人数据  自己的地址（接收校验地址）
...
u8   TX_ADDRESS[TX_ADR_WIDTH]= {0x34,0x43,0x10,0x10,0x01};	//发送给别人时  对方的地址（发送帧头地址）
u8   RX_ADDRESS[RX_ADR_WIDTH]= {0x34,0x43,0x10,0x10,0x02};	//接收别人数据  自己的地址（接收校验地址）
void NRF24L01_Test(void) 
{	
	u32 old=0,error=0;
...
u8 NRF24L01_Check(void)
{
	...
	NRF24L01_Read_Buf(NRF_WRITE_REG+TX_ADDR,buf,5);//读出写入的地址  
...
{						
	...
	NRF24L01_Write_Buf(NRF_WRITE_REG + RX_ADDR_P0, (u8*)TX_ADDRESS, TX_ADR_WIDTH);   //只能用接收通道0接收应答，接收通道0地址要写成对方地址 	
