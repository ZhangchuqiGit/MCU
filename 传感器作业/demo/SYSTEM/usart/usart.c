...
/*------   数据 形式  "#$...%"    -------*/	
    if(	(USART2_RX_BUF[USART2_RX_num-2] == '#')&&(USART2_RX_BUF[USART2_RX_num-1] == '&') ) //数据头 "#&..."
		USART2_RX_head = USART2_RX_num-2;   //头   					      #&0001%	
    USART2_RX_len = USART2_RX_num-1- USART2_RX_head; //长度  			//0123456											
    if( (USART2_RX_BUF[USART2_RX_head] == '#')&&(USART2_RX_BUF[USART2_RX_num-1] == '%')&&USART2_RX_len==6) //检测到头的情况下检测到尾   
    {   
        USART2_RX_num = 0;    
		USART2_led=1; //收到语音			
    }   
