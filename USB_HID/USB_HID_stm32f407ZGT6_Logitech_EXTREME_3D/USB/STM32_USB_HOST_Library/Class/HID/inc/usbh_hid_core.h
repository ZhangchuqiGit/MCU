...
typedef struct _HID_cb // 用户 USB 设备 初始化、驱动
{
	void  (*Init)   (void);
	void  (*Decode) (uint8_t *data/*所求数据*/);

} HID_cb_TypeDef;

/* Structure for HID process */
typedef struct _HID_Process // HID 流程
{
	uint8_t              buff[64];/*数据*/
	uint8_t              hc_num_in;
	uint8_t              hc_num_out;
	HID_State            state;
	uint8_t              HIDInt_Out_Ep;
	uint8_t              HIDInt_In_Ep;
	HID_CtlState         ctl_state;
	uint16_t             length;
	uint8_t              ep_addr;
	uint16_t             poll;
	__IO uint16_t        timer;
	HID_cb_TypeDef       *cb;  // 用户 USB 设备 初始化、驱动
} HID_Machine_TypeDef;
...
/** @defgroup USBH_HID_CORE_Exported_Variables
  * @{
  */
extern USBH_Class_cb_TypeDef  CLASS_cb;//类回调结构地址
/**
  * @}
  */
