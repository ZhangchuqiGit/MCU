...
__ALIGN_BEGIN HID_Machine_TypeDef        HID_Machine 	__ALIGN_END ; // HID 流程
...
static void  USBH_ParseHIDDesc (USBH_HIDDesc_TypeDef *desc, const uint8_t *buf);
...
static USBH_Status USBH_HID_InterfaceInit (USB_OTG_CORE_HANDLE *pdev, void *phost)
{
	...
	if(pphost->device_prop.Itf_Desc[0].bInterfaceSubClass/*接口子类*/ == HID_BOOT_CODE

	   /*适配 Logitech Extreme 3D 飞行摇杆*/
	   || pphost->device_prop.Itf_Desc[0].bInterfaceSubClass == 0 )
	{
		/* Decode Bootclass Protocl: Logitech Extreme 3D 飞行摇杆 or Mouse or Keyboard */
		if(pphost->device_prop.Itf_Desc[0].bInterfaceProtocol/*接口类协议*/
		   == HID_Logitech_BOOT_CODE) // 适配 Logitech Extreme 3D 飞行摇杆
		{
			/* 用户 USB 设备 初始化、驱动 */
			HID_Machine.cb = &HID_Logitech_cb;
		}
		else if(pphost->device_prop.Itf_Desc[0].bInterfaceProtocol/*接口类协议*/
		...
		if (HID_Machine.poll < HID_MIN_POLL) HID_Machine.poll = HID_MIN_POLL; // 10 ms
		...
		for (num=0; num < maxEP; num++)
		{
			...
			{
				HID_Machine.HIDInt_In_Ep = (pphost->device_prop.Ep_Desc[0][num].bEndpointAddress);
				HID_Machine.hc_num_in
						= USBH_Alloc_Channel (pdev, pphost->device_prop.Ep_Desc[0][num].bEndpointAddress);
			...
			{
				HID_Machine.HIDInt_Out_Ep = (pphost->device_prop.Ep_Desc[0][num].bEndpointAddress);
...
static USBH_Status USBH_HID_ClassRequest(USB_OTG_CORE_HANDLE *pdev, void *phost)
{
	...
	switch (HID_Machine.ctl_state)
	{
		...
		case HID_REQ_SET_IDLE:
			//这里duration官方设置的是0,修改为100,提高兼容性
	...
	switch (HID_Machine.state)
	{
		...
		case HID_POLL:
			...
			else if(HCD_GetURB_State(pdev , HID_Machine.hc_num_in) == URB_DONE)
			{
				...
				{
					start_toggle = 0;
					HID_Machine.cb->Decode(HID_Machine.buff/*所求数据*/);
				}
			}
								  ...
								  uint8_t reportId)
{
	...
	//官方的这里设置的是0,导致部分鼠标无法识别,这里修改为100以后,识别率明显提高.
