...
*         Initialize core registers address. 初始化核心寄存器地址
* @param  pdev : Selected device
...
*         Disenables the controller's Global Int in the AHB Config reg
* @param  pdev : Selected device
* @retval USB_OTG_STS : status
*/
...
#ifndef USE_OTG_MODE
	USB_OTG_DriveVbus(pdev, 1);				// ��USB�˿ڹ���
#endif
...
uint32_t USB_OTG_ResetPort(USB_OTG_CORE_HANDLE *pdev)
{
	...
	hprt0.b.prtrst = 1;
	USB_OTG_WRITE_REG32(pdev->regs.HPRT0, hprt0.d32);
	USB_OTG_BSP_mDelay (9);                                /* See Note #1 */
	hprt0.b.prtrst = 0;
	USB_OTG_WRITE_REG32(pdev->regs.HPRT0, hprt0.d32);
	USB_OTG_BSP_mDelay (19);
	return 1;
