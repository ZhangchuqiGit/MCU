...
/**
  ...
  */
USBH_Status USBH_DeInit(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)//初始化主机
{
	/* Software Init */
	phost->gState = HOST_IDLE; 		// 主机状态: 主机空闲
	phost->gStateBkp = HOST_IDLE;	// 先前的主机状态备份: 主机空闲
	phost->EnumState = ENUM_IDLE;	// 枚举状态: 枚举空闲
	phost->RequestState = CMD_SEND;	// 请求状态: 请求发送

	/* 控制类型 */
	phost->Control.state = CTRL_SETUP; // 设置
	phost->Control.ep0size = USB_OTG_MAX_EP0_SIZE;	// 数据大小
			   ...
			   USBH_Usr_cb_TypeDef *usr_cb) // 用户回调结构地址
{
	...
	/*Register class and user callbacks 注册类和用户回调 */
	...
	switch (phost->gState)/* Host State */
	{
		case HOST_IDLE :					//主机空闲
			...
			if (HCD_IsDeviceConnected(pdev))
			{
				...
				USB_OTG_BSP_mDelay(100);//若检测到设备插入，复位总线100ms
