...
{
	uint8_t i = 0;
	pdev->host.ConnSts = 0; // 检查设备是否已连接: 否
...
{
	/* 在开始驱动USB复位之前，应用程序等待由去抖完成位（OTG_FS_GOTGINT中的DBCDNE位）触发的OTG中断，
	 * 这表明总线在由于上拉电阻的连接而引起的电去抖之后再次稳定在DP（FS）或DM（LS）上。
...
{
	return (pdev->host.ConnSts); // 检查设备是否已连接
}
...
/**
  ...
  *         This function returns the last XferCnt
  * @param  pdev: Selected device
...
/**
  ...
  *         This function prepare Submit Request
  * @param  pdev: Selected device
  * @param  hc_num: Channel number 
  * @retval status    */
