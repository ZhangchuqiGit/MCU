...
#define EN_USART_code_key		1		//使能（1）/禁止（0）  数据包的形式接收
...
接收状态 :
	...
	extern char  USART1_RX_BUF[USART1_REC_LEN]; //接收缓冲,最大USART1_REC_LEN个字节.末字节为换行符 
	...
	#endif
#endif
#if EN_USART1_RX   ////如果串口2使能了接收
	void uart2_init(u32 bound);
	extern char  USART2_RX_BUF[USART2_REC_LEN]; //接收缓冲,最大USART1_REC_LEN个字节.末字节为换行符 
	#if EN_USART_code_key // 数据包的形式接收
	...
	extern char  USART3_RX_BUF[USART3_REC_LEN]; //接收缓冲,最大USART1_REC_LEN个字节.末字节为换行符 
	#if EN_USART_code_key // 数据包的形式接收
