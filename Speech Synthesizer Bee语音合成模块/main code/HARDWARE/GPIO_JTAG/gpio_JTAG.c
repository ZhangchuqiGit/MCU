...
*/
void JTAG_SWD_GPIO_Config()//stm32的PB3 PB4 PA15引脚可以在使用SWD仿真和烧录的情况下释放出来，作为普通IO口使用
{
 /*   //打开重映射时钟和USART重映射后的I/O口引脚时钟
  ...
  //  AFIO->MAPR=0;
    ...
    /*这段初始化代码放在所有IO初始化最后！！！！！！
    当把PB3 PB4 PA15(JTAG=SWD+ PB3+PB4+PA15)初始化好之后，如果再次调用GPIOA GPIOB接口的时钟代码的话！会使得PB3 PA15引脚变回JTAG的引脚
即初始化完之后不能再执行如下代码，PB3 PA15引脚会变回JTAG的引脚，而无法作为普通IO使用*/
