...
float AngleOffset_Rol=0.0,AngleOffset_Pit=0;

void Accel_Con(Int16_xyz *acc_in,Int16_xyz *acc_out)//加速度滤波
{
	static uint8_t 	filter_cnt=0;
	static int16_t	ACC_X_BUF[FILTER_NUM]={0},ACC_Y_BUF[FILTER_NUM]={0},ACC_Z_BUF[FILTER_NUM]={0};
...
//void IMUupdate( &Gyro/*角速度*/, &Acc_Data_Con/*滤波后的加速度*/, &Att_Angle/*ATT函数计算出的姿态角*/);//IMU姿态解算
void IMUupdate(Int16_xyz *gyr, Int16_xyz *acc, Float_angle *angle)
{
	float ax = acc->X,ay = acc->Y,az = acc->Z;//加速度
	float gx = gyr->X,gy = gyr->Y,gz = gyr->Z;//角速度
	float norm;
...
//  float q1q2 = q1*q2;
	...
	ax = ax / norm;//  arctan(x/l)  
	ay = ay / norm;//  arctan(y/l)
	az = az / norm;//  arctan(z/l) 
	// estimated direction of gravity and flux (v and w)      估计重力方向和流量/变迁
	// 这是把四元数换算成《方向余弦矩阵》中的第三列的三个元素。
	// 根据余弦矩阵和欧拉角的定义，地理坐标系的重力向量，转到机体坐标系，正好是这三个元素。
	// 所以这里的 vx\y\z，其实就是当前的欧拉角（即四元数）的机体坐标参照系上，换算出来的重力单位向量。      
	vx = 2*(q1q3 - q0q2);												
	vy = 2*(q0q1 + q2q3);
