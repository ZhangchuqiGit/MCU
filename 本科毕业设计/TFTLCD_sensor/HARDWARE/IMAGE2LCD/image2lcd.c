...
{
	HEADCOLOR *imginfo=(HEADCOLOR*)imgx;	//得到文件信息 
 	u8 ifosize=sizeof(HEADCOLOR);//得到HEADCOLOR结构体的大小
	POINT_COLOR=RED;   	
//	if(imginfo->w>=lcddev.width||imginfo->h>=lcddev.height)//图片大小超过整个屏幕
//	{
//		LCD_ShowString(10,70,200,16,16,"The Picture is too large");	//超过屏幕尺寸了
//		delay_ms(500);
//	}
//	else if((x+imginfo->w)>lcddev.width)//超过屏幕尺寸了
//	{
//		LCD_ShowString(10,70,200,16,16,"x > lcddev.width");	//超过屏幕尺寸了
//		delay_ms(500);
//	}
//	else if((y+imginfo->h)>lcddev.height)//超过屏幕尺寸了
//	{
//		LCD_ShowString(10,70,200,16,16,"y > lcddev.height");	//超过屏幕尺寸了
//		delay_ms(500);
//	}
//	else image_show(x,y,imginfo->w,imginfo->h,imginfo->scan,imgx+ifosize);		
