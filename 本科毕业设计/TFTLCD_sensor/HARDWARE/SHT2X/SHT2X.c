...
/**
  ...
  */
FunctionalState SHT2X_IIC_WriteByte(u8 WriteAddress,u8 SendByte)
{		
  ...
  */
FunctionalState SHT2X_IIC_ReadByte( u8 ReadAddress, uint16_t length  ,u8* pBuffer)
{		
	  if(!I2C_Start())return DISABLE;
...
{
		if(SHT2x_Calc_T())  /*湿度测试*/
		{ 
			 printf("温度：%f\r\n",temperatureC);
		   //OLED_shuji(38,17,(s8)(temperatureC));
		}
		if(SHT2x_Calc_RH())    /*湿度测试*/
		{
			printf("湿度：%f%%\r\n",humidityRH);
		}
}
