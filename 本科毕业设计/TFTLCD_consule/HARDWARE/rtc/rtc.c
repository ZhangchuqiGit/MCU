...
_calendar_obj calendar,time001;//时钟结构体 
...
{	
		 ...
		 NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;  //抢占优先级 //比RTC全局中断的优先级高
...
//返回0:正常
//其他:错误代码
//RTC_Init(0/*RTC秒中断*/,1/*RTC闹钟秒中断*/);
u8 RTC_Init(FunctionalState RTC_SEC_ENABLE_DISABLE,FunctionalState RTC_IT__ENABLE_DISABLE)
{		
	u8 temp = 0;
...
//	}
//	if(PWR_GetFlagStatus(PWR_FLAG_WU)!=RESET) //如果现在处于停机模式
//	{
//		PWR_ClearFlag(PWR_FLAG_WU);//清除停机模式
//	}
...
//		RTC_ITConfig(RTC_IT_ALR, ENABLE);//使能RTC闹钟秒中断
		RTC_ITConfig(RTC_IT_SEC, RTC_SEC_ENABLE_DISABLE);//使能RTC秒中断
		RTC_ITConfig(RTC_IT_ALR, RTC_IT__ENABLE_DISABLE);//使能RTC闹钟秒中断
...
//		printf("time_set_ok\r\n"); 
		...
		RTC_Alarm_Set(2020,3,1,1,1,10);  //设置闹钟 		
