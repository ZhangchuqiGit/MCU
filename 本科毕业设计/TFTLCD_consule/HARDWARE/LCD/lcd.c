...
//LCD的画笔颜色和背景色	   
u16 POINT_COLOR=0x0000;	//画笔颜色 设置字体为红色 
...
//写寄存器函数
//regval:寄存器值
void LCD_WR_REG(u16 regval)//LCD_WriteCmd
{   
...
void LCD_ShowChar_user(u16 x,u16 y,char num,u8 size,u8 mode) //显示一个字符
{
    u8 temp,t1,t;
	u16 y0=y;
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_ShowString_user(u16 x,u16 y,u8 size,char *p) 
{         
	u8 x0=x;	
	u16 width = strlen(p)*size;// sizeof(u8);// 计算数组中数据长度
	u16 height=size;
	width+=x;
	height+=y;
    while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
...
#if 0
/****************************************************************************
* 名    称：void LCD_Draw_Picture(u16 xstr,u16 ystr,u16 xend,u16 yend,u16 color_user,u8 *pic)
* 功    能：在指定位置显示一幅 color_user 单色图
* 入口参数：StartX     行起始座标
*           StartY     列起始座标
*           EndX       行结束座标
*           EndY       列结束座标
...
****************************************************************************/
void LCD_Draw_Picture(u16 xstr,u16 ystr,u16 xend,u16 yend,u16 color_user,u8 *pic)
{
	u8 temp,t1;
    ...
    {	
			...
			for(t1=0;t1<8;t1++)
			{
				if(temp&0x80)LCD_Fast_DrawPoint(x_t,y_t,color_user);//画实心点
...
{	
	...
	if(Dir==2) LCD_WriteReg(0x0003,0x1010);   //图像显示方向为右下起  行递减  列递增  AM=0  I/D[1:0]=00	<--
	else if(Dir==3) LCD_WriteReg(0x0003,0x1028);   //图像显示方向为右上起  行递减  列递增  AM=1  I/D[1:0]=10	V
	if(Dir==0)
	{
		LCD_WriteReg(0x0003,0x1030);		  //图像显示方向为左上起  行递增  列递增  AM=0  I/D[1:0]=11  -->
		LCD_WriteReg(0x0210, StartX); 	  //水平显示区起始地址 0-239
		LCD_WriteReg(0x0211, StartX+X-1);   //水平显示区结束地址 0-239
		LCD_WriteReg(0x0212, StartY);       //垂直显示区起始地址 0-399
		LCD_WriteReg(0x0213, StartY+Y-1);   //垂直显示区结束地址 0-399
		LCD_WriteReg(0x0200, StartX);		  //水平显示区地址
		LCD_WriteReg(0x0201, StartY);		  //垂直显示区地址	
		...
		{	
			...
			for(num=0; num<8; num++){	  //数组的每个字节代表了8个像素	    
				if((temp&0x80)>0){		  //对字节的各位进行判断，为1的用带入参数的16位颜色值标示，写入到像素位置。						 
					...
					LCD_WriteReg(0x0200, StartX+x);//水平显示区地址
				LCD_WriteReg(0x0201, StartY+y);//垂直显示区地址								
	...
	{
		LCD_WriteReg(0x0003,0x1018);//图像显示方向为左下起  行递增  列递减  AM=1  I/D[1:0]=01	A
		LCD_WriteReg(0x0210, StartY);//水平显示区起始地址 0-239
		LCD_WriteReg(0x0211, StartY+Y-1);//水平显示区结束地址 0-239
		LCD_WriteReg(0x0212, 399-(StartX+X-1));//垂直显示区起始地址 0-399
		LCD_WriteReg(0x0213, 399-StartX);//垂直显示区结束地址 0-399
		LCD_WriteReg(0x0200, StartY);//水平显示区地址
		LCD_WriteReg(0x0201, 399-StartX);//垂直显示区地址	
		LCD_WR_REG(0x0202);//准备写数据显示区
		...
		{					  
			...
			{	  	
				if((temp&0x80)>0){		  		//对字节的各位进行判断，为1的用带入参数的16位颜色值标示，写入到像素位置。						 
					...
					LCD_WriteReg(0x0200, StartY+y);		//水平显示区地址
					LCD_WriteReg(0x0201, 399-(StartX+x));	//垂直显示区地址								
					LCD_WR_REG(0x0202);					//准备读数据显示区
					R_dis_mem=LCD_RD_DATA();		  	//读取背景色，为叠加产生透明效果作准备	
					LCD_WR_DATA(R_dis_mem); 		//对字节的各位进行判断，为0的用当前背景像素16位颜色值标示。 						
				}
